...
function CompWidget({ settings, onRefresh }: { settings: AppSettings; onRefresh: () => void }) {
  ...
  const preview = {
    comp: Math.min(spentNum, limit),
    over: Math.max(0, spentNum - limit),
  };
  ...
  return (
    <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <p className="font-semibold text-indigo-800">🎫 Добавить трату по компенсации</p>
        <div className="flex rounded-lg overflow-hidden border border-indigo-200 text-xs">
          {[1, 2].map(n => (
            <button key={n} onClick={() => setPersons(n)}
              className={`px-3 py-1.5 font-medium transition ${persons === n ? 'bg-indigo-600 text-white' : 'bg-white text-indigo-600 hover:bg-indigo-50'}`}>
              {n === 1 ? '1 чел.' : '2 чел.'}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-2">
        <input type="number" min="0" step="any" value={spent} onChange={e => setSpent(e.target.value)}
          placeholder={`Потратили на обед, ₽ (лимит ${fmt(limit)})`}
        ...
        <input type="date" value={date} onChange={e => setDate(e.target.value)}
          className="border border-indigo-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white" />
        <button onClick={handleAdd} disabled={loading || !spent || spentNum <= 0}
          className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-semibold text-sm hover:bg-indigo-700 transition disabled:opacity-40">
          {loading ? '...' : 'OK'}
        </button>
      </div>
      ...
      {spentNum > 0 && !result && (
        <div className="flex gap-3 text-xs">
          <span className="text-green-700 bg-green-50 rounded-lg px-2 py-1">
            +{fmt(preview.comp)} компенсация
          </span>
          {preview.over > 0 && (
            <span className="text-orange-700 bg-orange-50 rounded-lg px-2 py-1">
              −{fmt(preview.over)} в Продукты (сверх лимита)
            </span>
          )}
        </div>
      )}
      ...
      {result && (
        <div className="flex gap-3 text-xs">
          <span className="text-green-700 bg-green-100 rounded-lg px-2 py-1">
            ✓ +{fmt(result.compAmount)} компенсация добавлена
          </span>
          {result.overAmount > 0 && (
            <span className="text-orange-700 bg-orange-100 rounded-lg px-2 py-1">
              ✓ −{fmt(result.overAmount)} записано в Продукты
            </span>
          )}
        </div>
      )}
